...
** Last update Sun Jun  4 15:52:03 2017 Manar Benkraouda
*/
...
float	algo(t_car *car)
{
  ...
  if (car->lidar[15] >= 1750.0)
    ret = (car->lidar[1] - car->lidar[31]) < 0 ? 0.005 : -0.005;
  else if (car->lidar[15] >= 1250.0)
    ret = (car->lidar[1] - car->lidar[31]) < 0 ? 0.05 : -0.05;
  else if (-car->lidar[15] >= 750.0)
    ret = (car->lidar[1] - car->lidar[31]) < 0 ? 0.1 : -0.1;
  else if (car->lidar[15] >= 550.0)
    ret = (car->lidar[1] - car->lidar[31]) < 0 ? 0.2 : -0.2;
  else if (car->lidar[15] >= 350.0)
    ret = (car->lidar[1] - car->lidar[31]) < 0 ? 0.3 : -0.3;
  else
